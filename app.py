...
def format_publish_error(response: requests.Response) -> str:
    """Create a useful API error message for failed publishing."""
    try:
        payload = response.json()
    except Exception:
        payload = {}

    # Reverb-style payloads can include message/errors/error fields.
    details = []
    if isinstance(payload, dict):
        ...
        if isinstance(errors, list):
            details.extend([str(err) for err in errors[:3]])
        elif errors:
            details.append(str(errors))

    detail_text = " | ".join([d for d in details if d]).strip()
    if detail_text:
        return f"Error: {response.status_code} - {detail_text}"
    return f"Error: {response.status_code} - {response.text[:140]}"
...
def publish_draft(api_key: str, listing_id: Any) -> Tuple[bool, str]:
    ...
    if details:
        listing_state = details.get("state", {}).get("description", "Unknown")
        listing_errors = details.get("errors")
        if listing_errors:
            return False, f"❌ Publish failed. Status: {listing_state}. Missing/invalid fields: {listing_errors}"
        return False, f"❌ Publish failed. Status: {listing_state}. Listing may still be incomplete."
    if details_error:
        return False, f"❌ Could not publish - all endpoints failed ({details_error})"
...
def is_listing_publish_ready(draft: Dict[str, Any]) -> Tuple[bool, List[str]]:
    ...
    # Condition is frequently required to publish.
    condition = str(draft.get("condition", "")).strip()
    if not condition:
